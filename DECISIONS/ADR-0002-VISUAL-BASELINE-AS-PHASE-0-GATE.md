# ADR-0002 — 视觉设计基线作为 Phase 0 退出门禁（Visual Baseline as Phase 0 Gate）

Status: PROPOSED
ADR Number: ADR-0002
Title: 视觉设计基线作为 Phase 0 退出门禁（第四包阶段边界治理）
Date: 2026-07-21
...
1. V1.1 与第三包（UX / UI / 交互契约）已经定义 Control Room、交互、状态语义与设计系统，但没有把"高保真视觉基线"明确列为 Phase 0 的强制退出门禁。
2. 本提案在 `ROADMAP.md` 与退出矩阵（第 5 节主矩阵）中新增第四包（视觉设计基线与视觉资产治理包），属于阶段门禁细化，必须经过独立 ADR 审核与 owner 批准，不能静默生效。
3. 当前所有第四包条件（`P4-VISUAL-BRIEF` / `P4-CONTROL-ROOM-BASELINE` / `P4-VISUAL-ASSET-MANIFEST` / `P4-REVIEW` / `P4-OWNER`）均处于 `NOT_STARTED`，尚未开始。

## 2. 决策（Decision）

1. 将"视觉设计基线与视觉资产治理包"登记为 Phase 0 实质性退出条件。
...
6. 即使本 ADR 获批，Phase 0 仍保持 `NOT_READY` / `OPEN`，直到第四包真实完成。

## 3. 替代方案（Alternatives）

- **把视觉基线推迟到 Phase 1**：拒绝。可能造成实现前缺少稳定视觉基准及返工。
- **只把视觉方案作为非阻塞参考**：拒绝。无法形成可审核、可追溯的实施基准。

## 4. 后果（Consequences）

- **正面**：把视觉设计基线确立为可审核、可追溯的 Phase 0 强制门禁，避免实现阶段返工与基准漂移。
- **负面 / 风险**：增加 Phase 0 退出前置条件，延长 Phase 0 周期；依赖独立 design lead 资源。
- **后续义务**：第四包三项设计产物与两项治理证据须真实完成，并经独立审核与 owner 批准，Phase 0 方可关闭。

## 5. 证据（Evidence）

- V1.1 全文（已批准 SoT）与第三包（UX / UI / 交互契约）。
- `ROADMAP.md`（第四包登记）。
- `PHASE_0_EXIT_CRITERIA.md` 第 4、5 节（第四包状态与主矩阵 `P4-*`）。
