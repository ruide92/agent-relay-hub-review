...
`conformance/` 下的 JSON fixtures 为非执行型设计验收向量，用于表达"该校验路径必须接受/拒绝"。当前 81 个 fixtures（28 valid + 53 invalid），详见 `MACHINE_READABLE_CONTRACTS.md` §10。
...
- Fixture count：**81** 个 conformance fixtures（28 valid + 53 invalid）

### 分层 Validation Status

| 层 | 状态 | 说明 |
|---|---|---|
| Static Precheck | `PASS` | JSON 语法（93 文件 = 12 schema + 81 fixture）、$id 唯一性（12 ID）、$ref 可解析性（99 ref）、duplicate-key 检测（93 文件，0 重复键）、atomic preflight（UUIDv4/hash/base64url/required） |
| Schema Meta-Validation | `PASS` | 12/12 Schema 通过 Ajv 8.20.0 Draft 2020-12 编译（strict=false；含 contains-without-type 与相对 $ref 兼容） |
| Fixture Execution | `PASS` | 28/28 valid fixtures 通过对应 Schema instance validation；53/53 invalid fixtures 被正确处理（schema-layer 被拒绝；business/decoded-header-layer 为 schema-valid 但被对应业务层拒绝） |
...
> **诚实声明**：Schema Meta-Validation 与 Fixture Execution 已在外部验证环境 `E:\omp-tools\json-schema-audit`（Ajv 8.20.0 + ajv-formats 3.0.1, Draft 2020-12）中运行，**未在仓库安装任何依赖**。strict=false 用于 Draft 2020-12 `contains`-without-`type:array` 兼容与相对 `$ref` 文件解析；这不影响 Draft 2020-12 规范合规性。Crypto/Runtime Validation 仍 `UNVALIDATED`。
