...
- 严重度无法确定时，高风险动作阻断；低风险问题可进入 deferred，不是一律提升到最高严重度；
...
### R2-03 Qoder 实现路线（P1）
Phase 2 优先验证：Qoder CLI；Qoder headless；Qoder Agent SDK；结构化输出；流式事件；取消和恢复；工具权限；进程退出码；会话关联。桌面 UIA 只作为 Qoder CLI/SDK 无法覆盖功能的补充通道。
...
### R2-05 事务型 Outbox（P0）
消息投递与内部状态变更必须通过事务型 Outbox 或等价机制衔接，避免：
- 数据库显示已发送，但外部未收到；
- 外部已执行，但内部没有记录；
- 崩溃后盲目重复发送。
...
### R2-09 外部工具权威边界（P0）
WorkBuddy / Qoder 等外部 messenger、worker 或 reviewer 都不是状态权威。唯一权威状态必须留在 ARH 的事件账本和政策引擎中。外部工具只能返回声明与证据，不能决定：
- 当前工作流状态；
- 是否通过发布门禁；
- 是否重置循环次数；
- 是否提高权限；
