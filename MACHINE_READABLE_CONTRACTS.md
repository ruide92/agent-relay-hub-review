...
Validation Status: UNVALIDATED（设计契约，未实现、未运行验证）
```
...
## 2. JSON Schema dialect 与版本规则

1. **dialect**：全部 Schema MUST 使用 **JSON Schema Draft 2020-12**，并在每个 Schema 中以 `$schema: "https://json-schema.org/draft/2020-12/schema"` 显式声明。
2. **选择理由**：Draft 2020-12 是当前广泛实现的稳定 dialect；ARH 技术栈（.NET 10 LTS / JSON）存在成熟实现；无更强理由引入更新或更老 dialect（见 `DECISIONS/ADR-0003-…` 对 token 格式的同类标准化裁决）。
3. **dialect fail-closed**：`$schema` 缺失、不可识别或非 2020-12 时，校验方 MUST 拒绝，MUST NOT 按猜测 dialect 解析。
4. **schema_version**：每个契约对象实例 MUST 携带 `schema_version`（语义化 `MAJOR.MINOR.PATCH`），与 PROTOCOL §3 一致；Schema 文件自身的版本在 `$id`（`/v1/` 段）与 `title` 中标识。
5. **兼容性规则**：
   - 同 `MAJOR` 内：仅允许新增**可选**字段、扩展枚举须同步更新注册表（见 §5）与 prose；
   - 任何删除字段、收窄类型、修改语义的变更为 breaking，MUST 升 `MAJOR`（`/v2/`），并由治理提案批准；
   - 实例的 `schema_version` 不在当前支持集合内时 MUST 拒绝（fail-closed），不得静默按最近版本解析。
