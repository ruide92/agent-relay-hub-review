...
## 8. 密钥与信任根（已批准的 Phase 1 签名设计基线）

> 以下为 Phase 1 **签名设计基线**，已由 Reviewer-2 审核并经 `GOVERNANCE_APPROVAL_0002.md` 批准（作为第二包规范性 Source of Truth 的一部分）；批准的是**设计基线本身**。
> 本文档不写入任何真实密钥或 secret；签名、信任根、轮换、撤销与 safe mode 当前**尚未实现、尚未验证**，`Validation Status` 仍为 `UNVALIDATED`。参数变化必须通过安全 ADR。
...
### 8.1 Capability Token 签名容器（GOV-APP-0007，APPROVED DESIGN）

> 本小节已随 target `19fdff1beaa5208fec23653f83b47046fe8c3427` 通过独立审核，并由 `GOVERNANCE_APPROVAL_0007.md` 批准为 Phase 0 设计契约。
> 所有运行时机制当前 **未实现、未验证**，`Validation Status` 仍为 `UNVALIDATED`。

- Capability token 使用 **独立签名容器**（JWS Compact Serialization，RFC 7515），**不使用** detached signature manifest。
  - 理由：capability token 需要跨系统验证与标准库互操作，JWS 提供自包含的 protected header；detached manifest 适用于覆盖大型文档（policy bundle / SBOM），不适用于短生命周期 token。
- `key_purpose` 固定为 `capability_token`，**MUST NOT** 与 `policy_bundle` 或 `sbom` 共用（§8 用途分离）。
- 签名算法：**ES256**（ECDSA P-256 / SHA-256），与 §8 基线一致。
- `alg=none` **绝对禁止**。
- Protected header 额外含 `key_purpose` 字段（固定 `capability_token`）。
- Payload canonicalization 使用 **JCS（RFC 8785）**，与 envelope 层的 canonical JSON 各自独立。
- 完整 token 引用规则：在 ARP / Event Ledger / 日志 / 截图中 **MUST NOT** 出现完整 token；MUST 仅含 `jti` 与 token 哈希。
- Replay protection：已接受的 `jti` replay 状态 MUST 写入 Event Ledger / SQLite（持久化），不使用仅进程内缓存；保留期覆盖 `exp + clock_skew`；服务恢复时 replay 状态重建完成前 fail-closed。
- Revocation：撤销清单为 content-only（不内嵌 signature），通过独立 sidecar signature manifest 签名（`key_purpose=capability_token_revocation`）；`list_version` 单调递增防回滚。
- Key rotation：须有重叠验证窗口（old+new 同时可信），与 §8 既有规则一致。
- Clock skew：Token 被接受当且仅当 `nbf - clock_skew <= now <= exp + clock_skew`；候选默认 60 秒；policy bundle MUST 可收紧但 MUST NOT 放宽超过 300 秒。
- Policy bundle / revocation 签名模型：内容文件与 sidecar signature manifest 分离；`target_hash` = 内容文件 JCS canonical bytes 的 SHA-256（无自引用排除规则）。
- 所有失败路径 **MUST fail-closed**。
- 详细 wire format、claims、校验顺序见 `DECISIONS/ADR-0003-…`（ACCEPTED）。

### 8.2 Detached Signature Manifest 规范化签名预映像（GOV-APP-0007，APPROVED DESIGN）

对 `policy_bundle`、`sbom` 与 `capability_token_revocation` 的每个 detached signature entry，签名方与验证方 MUST 使用唯一 profile `arh-detached-signature-v1`。不得直接签裸摘要、带前缀的哈希字符串或未定义字段子集。

1. 取 manifest 根字段 `schema_version`、`manifest_id`、`extensions`（缺失时按空对象 `{}`）以及当前 signature entry 的全部字段，但排除 `signature_value`。
2. 构造以下对象并按 RFC 8785 JCS 规范化：
...
```json
{
  ...
  "extensions": {},
  "key_id": "<entry.key_id>",
  "key_purpose": "<entry.key_purpose>",
  "manifest_id": "<manifest.manifest_id>",
  "preimage_profile": "arh-detached-signature-v1",
...
3. 精确签名预映像为：`UTF8("ARH-DETACHED-SIGNATURE-V1\n") || JCS_UTF8(上述对象)`。
4. `signature_value` = 对该预映像执行 ECDSA P-256 / SHA-256 后所得 raw `R||S`（64 bytes）的无填充 BASE64URL。
5. 验证方 MUST 由收到的 manifest 重新构造同一预映像并验签；任何 `schema_version`、`manifest_id`、`extensions`、target、hash、key、purpose、algorithm 或 `signed_at` 替换都会使验签失败。
6. `extensions` 不得承载密钥、授权决策或其他绕过封闭字段的安全语义；即使为空也必须以 `{}` 参与预映像。

该 profile 绑定 detached manifest 的全部安全相关元数据。Capability token 仍使用 JWS signing input（§8.1），不得使用本 profile。
...
- 安全设计基线已批准；所有运行时控制（含签名、信任根、轮换、撤销、safe mode、审计）当前均**未实现、未验证**；不得描述为已经实现或测试通过。
