...
> 所有运行时机制当前 **末实现、未验证**，`Validation Status` 仍为 `UNVALIDATED`。

- Capability token 使用 **独立签名容器**（JWS Compact Serialization，RFC 7515），**不使用** detached signature manifest。
  - 琾由：capability token 需要跨系统验证与标准库互操作，JWS 提供自包含的 protected header；detached manifest 适用于覆盖大型文档（policy bundle / SBOM），不适用于短生命周期 token。
...
- Replay protection：replay cache 以 `jti` 为键，TTL=`exp + clock_skew`；重放 MUST 被拒绝。
- Revocation：撤销清单独立签名（`key_purpose=capability_token_revocation`），`list_version` 单调递增防回滚。
- Key rotation：须有重叠验证窗口（old+new 同时可信），与 §8 既有规则一致。
- Clock skew 候选默认 60 秒；policy bundle MUST 可收紧但 MUST NOT 放宽超过 300 秒。
