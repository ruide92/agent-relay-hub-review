...
| UX-REQUEST-A5 | 请求 A5 批准 | A5 操作待授权 | 提交 A5 请求 + 范围 | `auth.a5_request` | A5 默认禁止；须 owner 批准 | `AWAITING_POLICY_DECISION` | 精确权限范围说明 | owner 批准/拒绝记录 | owner 未批→不执行（V1.1 §4.2） | `audit.event` |
...
- 超时后进入可解释的失败或陈旧态（如 `BLOCKED_*` / UI-STALE），**不得无限 loading**；
