...
| **operator** | Control Room、Workflows、Evidence、Reconciliation、Adapters、Budgets、Audit、Safe Mode | 创建模拟工作流、启动/暂停/取消/恢复、查看证据与 finding、触发备份/恢复演练、请求 A5 | 不得直接写 SQLite / projection / policy bundle / 终态；A5 默认禁止，须请求 | A0–A4（按预授权与令牌） |
| **reviewer** | 待审 workflow、finding、evidence | 提交 `review.finding`、请求重新验证、确认证据 | 不得改终态（仅提交待裁决事件，PROTOCOL.md §11）；不得执行外部动作 | A1–A3（审核/规划/候选） |
...
|---|---|---|---|---|---|---|---|---|---|---|
| UX-CREATE-SIM | 创建模拟工作流 | A2 范围内、mock adapter 可用 | 打开创建向导，填目标/政策 | `workflow.create_simulation` | A2（沙箱/任务分支），Phase 1 仅 mock | `PENDING` | 目标固化事件 + policy decision | `status: PENDING` 落 Event Ledger | `BLOCKED_*` / `POLICY_INTEGRITY_SAFE_MODE` | `audit.event` |
| UX-START | 启动工作流 | `PENDING` + 授权预检通过 | 点击启动 | `workflow.start` | A1–A3 + capability token | `RUNNING` | 权限预检 policy decision | `status: RUNNING` | `BLOCKED_AUTH_EXPIRED` / `BLOCKED_BUDGET_EXHAUSTED` | `audit.event` |
| UX-PAUSE | 暂停请求 | `RUNNING` | 发起暂停请求 | `workflow.pause_request` | A1 | `RUNNING`（暂停待确认） | adapter 最终状态事件 | `status: STALLED`（或恢复） | adapter 拒绝→保持 `RUNNING` | `audit.event` |
| UX-CANCEL | 取消请求 | 任意非终态 | 发起取消请求 | `workflow.cancel_request` | A1；A5 取消受保护须升级 | `RUNNING`（取消待确认） | adapter 报告最终动作状态（PROTOCOL §9） | `CANCELLED_BY_POLICY` | adapter 仍在执行→ `RECONCILIATION_REQUIRED` | `audit.event` |
| UX-RESUME | 恢复 | `STALLED` / 暂停 | 提交 resume token/checkpoint | `workflow.resume` | A1 | `RUNNING` | resume token + checkpoint（PROTOCOL §9） | 恢复成功事件 | 缺失 token→拒绝，保持原态 | `audit.event` |
...
| UX-REQUEST-REVERIFY | 请求重新验证 | finding 待裁决 | 发起重新验证 | `evidence.reverify_request` | A1 | 等待 `evidence` 事件 | verifier 输出 | `evidence` 事件 + policy decision | 仍高危→ `release blocked` | `audit.event` |
...
| UX-EXPORT-DIAG | 导出脱敏诊断 | A0 或 safe mode 内 | 导出脱敏包 | `diagnostic.export` | A0；safe mode 仅本地（SECURITY §7） | 导出中 | 脱敏后的诊断数据 | 导出完成事件 | 含 secret→拒绝（SECURITY §12） | `audit.event` |
| UX-VIEW-POLICY | 查看政策 | 有签名 policy 引用 | 打开 Policy Viewer | `policy.query` | A0 | 只读视图 | policy version / key_id | 派生视图 | 不可读→ safe mode 提示 | （只读） |
| UX-REQUEST-A5 | 请求 A5 批准 | A5 操作待授权 | 提交 A5 请求 + 范围 | `auth.a5_request` | A5 默认禁止；须 owner 批准 | 等待 owner 待办 | 精确权限范围说明 | owner 批准/拒绝记录 | owner 未批→不执行（V1.1 §4.2） | `audit.event` |
...
| UX-TRIGGER-BACKUP | 触发备份 | A0/A4 预授权 | 触发一致性备份 | `backup.trigger` | A0/A4 | backup running | backup set ID + 哈希 | backup 完成事件 | 哈希不一致→安全终态（ARCHITECTURE §8） | `audit.event` |
| UX-TRIGGER-RESTORE-DRILL | 触发恢复演练 | A0/A4 预授权 | 触发恢复演练 | `restore.drill` | A0/A4 | restore verification | 恢复成功判据（哈希+重放+对账） | 演练通过事件 | 不一致→不宣称成功 | `audit.event` |
...
| `DEGRADED` | 已降级 | adapter 能力部分丢失 | 重路由/降级服务 | 是 | 查看、暂停 | 强制提权 | capability_change 事件 | 恢复关键能力→`READY` |
...
| `POLICY_INTEGRITY_SAFE_MODE` | 安全模式 | 政策/信任根校验失败 | 仅本地 A0 诊断 | 是 | 查看、导出诊断 | A1–A5、外部调用 | 校验失败记录 | 可信政策重载校验通过 |
