...
| 每个 finding 修复轮数 | 2 | 5 |
| 每日模型预算（token / 美元） | 2,000,000 tokens / $10 | 10,000,000 tokens / $50 |
| 单工作流模型预算（token / 美元） | 500,000 tokens / $2 | 2,000,000 tokens / $10 |
| 全局每日预算（token / 美元） | 10,000,000 tokens / $50 | 50,000,000 tokens / $200 |
| 插件崩溃窗口 N（分钟） | 10 | — |
| 崩溃阈值 M（次数） | 3 | — |
| 熔断冷却时间（分钟） | 15 | — |
...
**Phase 1（Core Prototype）候选 SLO 与告警**：

- SLO：终态达成率 ≥ 99%（仅模拟工作流）；duplicate_logical_job_rate = 0（逻辑任务去重）；ledger_replay_consistency_rate = 100%；projection_rebuild_success_rate = 100%；recovery_success_rate ≥ 99%。
- 告警：reconciliation_required_rate > 0 触发待办（非告警风暴）；policy_denial_count 异常突增告警；recovery_success_rate < 99% 阻断发布。

**Phase 3（Product MVP）候选 SLO 与告警**：

- SLO：adapter_availability ≥ 99.5%；adapter_error_rate < 1%；workflow_duration_p95 < 8 h；budget_exhaustion_count 不超过每日预算阈值；credential_leak_detection_count = 0（检测到即高危告警）。
...
- **load / negotiate / serve / shutdown 的进入与退出条件**：load 成功后进入 `LOADING`；协商能力通过进入 `NEGOTIATING` → `READY`；服务中 `READY` ↔ `BUSY`；异常降级 `DEGRADED`；shutdown 经 `STOPPING` → `STOPPED`，异常终止为 `FAILED`；
...
- n8n 等 fair-code（Sustainable Use License）组件不直接嵌入商业核心，除非获得明确许可并精确核对条款；
- 每个插件独立声明许可证；
- 产品名称、图标和文案不侵犯外部厂商商标；
...
reviewer_2    = 安全审核模型
adjudicator   = 独立仲裁模型
worker        = qoder_cli（稳定 Agent SDK 桌面执行者；UIA 仅补充通道）
verifier      = 本地测试/CI
messenger     = ARH 内置中继
notifier      = 飞书摘要
```
