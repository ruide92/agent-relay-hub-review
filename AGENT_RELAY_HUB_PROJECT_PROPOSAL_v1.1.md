...
| 传输失败重试 | 3 次，指数退避（最大切换尝试次数默认 3，带切换退避，见 §16.1） |
| 会话恢复 | 1 次 |
| finding 讨论 | 1 轮 |
| finding 修复 | 2 轮 |
| 仲裁重试 | 1 次 |
| 重新设计 | 1 轮 |
| 模型费用 | 工作流预算（V1.1：分工作流/每日/全局三层预算，见 §9.1） |
| 最大执行时间 | 按工作流模板设置，必须有默认值与绝对上限（V1.1） |
...
- 超权限请求由政策引擎拒绝；
- 商业版本支持签名插件和可信发布者列表；
- **插件崩溃退避 + 能力降级（V1.1 新增，P1）**：同插件 N 分钟内崩溃超 M 次则停用并告警，不无限重启；运行中适配器丢失关键能力时，对在途 job 触发重路由或安全终态。
...
### 13.1 MVP 单机架构（Core Prototype 形态）
...
- **切换抖动防护（V1.1 新增，P1）**：增最大切换尝试次数（默认 3）+ 切换退避，避免主备之间反复横跳。
...
> **可观测性（V1.1 新增，P1）**：声明"当前仅单 job，并行为后续"；定义结构化指标/SLO/告警阈值（不止 UI 字段）。
...
交付：在 Phase 2 探针通过门禁后，才实现正式产品适配器和第一个真实代码开发闭环（如一个桌面执行者 + 一个浏览器审核者 + GitHub + 通知）。  
...
SDK 生命周期（load → negotiate → serve → shutdown）、错误码、取消、恢复、健康检查**必须在 Phase 0 文档化**（ARCHITECTURE / PROTOCOL 指针或骨架），作为 Phase 0 验收清单可勾选项。
...
3. 执行者 BUSY 时不会被打断；
4. 应用崩溃后可恢复状态且不重复外部写操作（Outbox 对账）；
5. 两位 reviewer 独立盲审，仲裁前互不可见；
6. AI 自称成功但测试失败时，发布门禁必须阻止（release gate 不接受纯文本"完成"）；
7. 达到两轮修复上限后进入明确安全终态；
...
14. 全局暂停和熔断有效；
15. 成功或阻塞后只发送一次摘要；
16. 子代理越权被拒绝并留痕（§4.2.2）；
17. 政策可信根损坏时进入 `POLICY_INTEGRITY_SAFE_MODE` 且仅本地 A0 可用（§15.5）。
